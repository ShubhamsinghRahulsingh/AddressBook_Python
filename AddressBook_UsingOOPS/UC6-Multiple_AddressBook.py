# UC6-Create multiple AddressBooks for multiple contacts
class Contact:
...
class AddressBook:
    ...
    def create_addressbook(self):
        mylist = list(contacts_list)
        addressbook_name = input("Enter the Name you want to give to AddressBook:")
        addressbook_dict.update({addressbook_name: mylist})
        print("AddressBook Created Successfully")
        contacts_list.clear()
        del mylist

    def display_addressbook(self):
        for data in addressbook_dict:
            print("AddressBook Name is:", data)
            for value in addressbook_dict[data]:
                print(str(value), "\n", "---------------------------")
            print("**********************************************************")
...
if __name__ == "__main__":
    ...
    while flag:
        ...
        match select:
            ...
            case 4:
                address.delete_contact()
            case 5:
                address.create_addressbook()
            case 6:
                address.display_addressbook()
            case 7:
                flag = False
                break
